...
func (runner *runner) EnsurePortProxyRule(args []string) (bool, error) {
	fmt.Println("Running netsh interface portproxy add v4tov4", "arguments", args)
...
func (runner *runner) DeletePortProxyRule(args []string) error {
	fmt.Println("Running netsh interface portproxy delete v4tov4", "arguments", args)
	out, err := runner.exec.Command(cmdNetsh, args...).CombinedOutput()
...
func (runner *runner) EnsureIPAddress(args []string, ip net.IP) (bool, error) {
	...
	if exists == true {
		fmt.Println("Not adding IP address, as it already exists", "IP", ipToCheck)
		return true, nil
	}

	// IP Address is not already added, add it now
	fmt.Println("Running netsh interface IPv4 add address", "IP", args)
	out, err := runner.exec.Command(cmdNetsh, args...).CombinedOutput()
	...
	if err == nil {
		...
		fmt.Println("Waiting until IP is added to the network adapter", "IP", ipToCheck)
		for {
...
func (runner *runner) DeleteIPAddress(args []string) error {
	fmt.Println("Running netsh interface IPv4 delete address", "IP", args)
	out, err := runner.exec.Command(cmdNetsh, args...).CombinedOutput()
...
func checkIPExists(ipToCheck string, args []string, runner *runner) (bool, error) {
	...
	fmt.Println("Searching for IP in IP dump", "IP", ipToCheck, "IPDump", ipAddressString)
